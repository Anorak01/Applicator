...
async def on_ready():
    ...
    activity = discord.Activity(
        name=f"{len(bot.guilds)} guilds", type=discord.ActivityType.listening
    )
...
def is_reviewer_or_admin():
    async def predicate(ctx):
        ...
        if ctx.user.guild_permissions.administrator:
            ...
            raise PermissionNeeded("reviewer")
...
async def help(ctx):
    embed = discord.Embed(
        title="Applicator Help",
        description="Applicator is an open-source Discord application bot that's easy to setup directly in Discord. \nEvery command is usable with /slash. \nWith the new permission system, you need editor or reviewer role to work with the bot.\nBeing Administrator overrides this requirement.",
    )
    embed.add_field(
        name="```/application create [name]```",
        value="Creates a new application",
        inline=False,
    )
    embed.add_field(
        name="```/application response_channel```",
        value="Sets the response channel for application. **Important**",
        inline=False,
    )
    embed.add_field(
        name="```/application remove [name]```",
        value="Removes specified application",
        inline=False,
    )
    embed.add_field(
        name="```/application list```", value="Lists all applications", inline=False
    )
    embed.add_field(
        name="```/application editor```",
        value="Opens editor for application",
        inline=False,
    )
    embed.add_field(
        name="```/application actions```",
        value="Opens action editor for application",
        inline=False,
    )
    embed.add_field(
        name="```/application set_editor_role```",
        value="Sets editor role. Users with this role can create, edit and remove applications and actions",
        inline=False,
    )
    embed.add_field(
        name="```/application set_reviewer_role```",
        value="Sets reviewer role. Users with this role can review applications",
        inline=False,
    )
    embed.add_field(
        name="```/start_button```",
        value="Creates start button for specified application",
        inline=False,
    )
...
async def invite(ctx):
    ...
    invite_button = discord.ui.Button(
        label="Invite", style=discord.ButtonStyle.link, url=INVITE_LINK
    )
    view.add_item(invite_button)
    embed = discord.Embed(
        title="Invite Me",
        color=0x70FF50,
        description="If you like the bot and want to invite it to other servers, click the button below",
    )
    ...
    try:
        ...
        await ctx.response.send_message(
            content="It looks like the bot owner didn't set up this link correctly"
        )
        raise e
...
async def support(ctx):
    view = discord.ui.View()
    invite_button = discord.ui.Button(
        label="Support server", style=discord.ButtonStyle.link, url=SUPPORT_LINK
    )
    view.add_item(invite_button)
    embed = discord.Embed(
        title="Support",
        color=0x70FF50,
        description="If you're having issues with the bot, you can join my support server with the button below",
    )
    embed.set_footer(text="Made by @anorak01", icon_url=owner_icon_url)
    try:
        await ctx.response.send_message(embed=embed, view=view)
...
async def review(ctx):
    ...
    invite_button = discord.ui.Button(
        ...
        url="https://top.gg/bot/1143622923136024767#reviews",
    )
    view.add_item(invite_button)
    embed = discord.Embed(
        title="Review",
        color=0x70FF50,
        description="If you like the bot, please consider leaving a review and upvote my bot, it will really help a lot!",
    )
    embed.set_footer(text="Made by @anorak01", icon_url=owner_icon_url)
    await ctx.response.send_message(embed=embed, view=view)
...
application = discord.SlashCommandGroup(
    "application", "The main command to manage applications"
)
...
async def create(ctx, application):
    if len(application) < 40:
        ...
        if result == "success":
            ...
            view.add_item(options)
            await ctx.response.send_message(
                f"Successfully created application: {application}\n\nPlease set a response channel:",
                ephemeral=True,
                view=view,
            )  # create a new application, modal with name ask
        else:
            await ctx.response.send_message(
                f"Application {application} already exists", ephemeral=True
            )
    else:
        await ctx.response.send_message(
            f"please choose a different name", ephemeral=True
        )
...
async def remove(ctx, application):
    ...
    if result == "success":
        await ctx.response.send_message(
            f"Successfully removed application: {application}", ephemeral=True
        )
    else:
        await ctx.response.send_message(
            f"Application {application} not found", ephemeral=True
        )
...
async def editor(ctx: discord.ApplicationContext):
    view = discord.ui.View()
    options = SelectApplicationOptionsEditor(
        max_values=1, placeholder="Select application"
    )
    applications = GuildAppDB.get_applications(str(ctx.guild.id))
    if applications:
        ...
        emb = discord.Embed(
            title="You haven't created any applications yet.",
            description="Create one using the `/application create` command",
        )
...
async def response_channel(ctx):
    view = discord.ui.View()
    options = SelectApplicationOptionsRespChannel(
        max_values=1, placeholder="Select application"
    )
    applications = GuildAppDB.get_applications(str(ctx.guild.id))
    if applications:
...
class SelectEditorRoleView(discord.ui.View):
    @discord.ui.select(select_type=discord.ComponentType.role_select, max_values=1)
    async def select_callback(self, select, interaction: discord.Interaction):
        self.disable_all_items()
        GuildAppDB.set_editor_role(
            str(interaction.guild.id), str(select.values[0].id)
        )  # select.values[0].id is the id of selected role
        await interaction.response.edit_message(
            content=f"You have selected {select.values[0].mention} as the editor role.\nUsers with this role will have access to the application creation, deletion, editor and actions.",
            view=None,
        )
...
class SelectReviewerRoleView(discord.ui.View):
    ...
    async def select_callback(self, select, interaction: discord.Interaction):
        self.disable_all_items()
        GuildAppDB.set_reviewer_role(
            str(interaction.guild.id), str(select.values[0].id)
        )  # select.values[0].id is the id of selected role
        await interaction.response.edit_message(
            content=f"You have selected {select.values[0].mention} as the reviewer role.\nUsers with this role will be able to review applications and accept or reject them.",
            view=None,
        )
...
class SelectApplicationStartButton(discord.ui.Select):
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name=f"Click the button below to start your application for: {self.values[0]}",
            value="",
            inline=False,
        )
        embed.set_footer(text="Made by @anorak01", icon_url=owner_icon_url)
        ...
        try:
            ...
            await interaction.response.edit_message(
                content="Missing access to send message", view=None
            )
            return
        await interaction.response.edit_message(
            embed=None, content="Application button created", view=None
        )
        StartButtonDB.add_start_msg(
            str(message.id), str(self.values[0]), str(interaction.guild.id)
        )
...
class SelectResponseChannel(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        self.disabled = True
        GuildAppDB.set_response_channel(
            str(interaction.guild.id), self.app_name, str(self.values[0].id)
        )
        await interaction.response.edit_message(
            content=f"Selected channel: {self.values[0].mention} for application: {self.app_name}",
            view=None,
        )
...
class ApplicationEditorView(discord.ui.View):
    ...
    @discord.ui.button(
        label="New", style=discord.ButtonStyle.green, custom_id="editor:add", row=0
    )
    ...
    @discord.ui.button(
        label="Remove", style=discord.ButtonStyle.red, custom_id="editor:remove", row=0
    )
    async def remove_question(self, button, interaction: discord.Interaction):
        ...
        options = RemoveQuestionSelect(
            max_values=1, placeholder="Select question to remove"
        )
        options.set_app_name(self.application_name)
        questions, length = GuildAppDB.get_questions(
            str(interaction.guild.id), self.application_name
        )
        if length == 0:
            await interaction.response.edit_message(view=view)
            return
    ...
    @discord.ui.button(
        label="Edit", style=discord.ButtonStyle.primary, custom_id="editor:edit", row=0
    )
    async def edit_question(self, button, interaction: discord.Interaction):
        view = ApplicationEditorView(str(interaction.guild.id), self.application_name)
        options = EditQuestionSelect(
            max_values=1, placeholder="Select question to edit"
        )
        options.set_app_name(self.application_name)
        questions, length = GuildAppDB.get_questions(
            str(interaction.guild.id), self.application_name
    ...
    @discord.ui.button(
        label="Move", style=discord.ButtonStyle.gray, custom_id="editor:move", row=0
    )
    async def move_question(self, button, interaction: discord.Interaction):
        view = ApplicationEditorView(str(interaction.guild.id), self.application_name)
        options = MoveQuestionSelect(
            max_values=1, placeholder="Select question to move"
        )
        options.set_app_name(self.application_name)
        questions, length = GuildAppDB.get_questions(
            str(interaction.guild.id), self.application_name
...
class RemoveQuestionSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        self.disabled = True
        GuildAppDB.remove_question(
            str(interaction.guild.id), self.app_name, int(self.values[0]) + 1
        )
...
class EditQuestionSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.edit_message(
            view=editor, message_id=interaction.message.id
        )
...
class MoveQuestionSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        options = MoveQuestionSelectNum(
            max_values=1, placeholder="Select place for move"
        )
...
def get_actions_embed(
    guild_id, application, action_type: ActionInteraction
) -> discord.Embed:
    embed = discord.Embed(
        title=f"Application: {application}",
    ...
    for i, que in enumerate(actions):
        if que["action_type"] == "add_role":
            ...
            embed.add_field(
                value=f"**{i + 1}. {que['display_type']}: {role}**",
                name="",
                inline=False,
            )
        else:
            embed.add_field(
                value=f"**{i + 1}. {que['display_type']}**", name="", inline=False
            )
    embed.set_footer(text="Made by @anorak01", icon_url=owner_icon_url)
    return embed
...
class ActionAcceptEditorView(discord.ui.View):
    ...
    async def add_action(
        ...
        for i, que in enumerate(usable_actions):
            options.add_option(
                label=f"{str(i + 1)}. {que}", value=str(usable_actions[que])
            )
        view.add_item(options)
        await interaction.response.edit_message(view=view)
    ...
    async def remove_action(self, button, interaction: discord.Interaction):
        view = ActionAcceptEditorView(str(interaction.guild.id), self.application_name)
        options = RemoveActionSelect(
            max_values=1, placeholder="Select action to remove"
        )
        options.set_app_name(self.application_name)
        ...
        for i, que in enumerate(actions):
            if que["action_type"] == "add_role":
                ...
                options.add_option(
                    label=f"{str(i + 1)}. {que['display_type']}: {role}", value=str(i)
                )
            else:
                options.add_option(
                    label=f"{str(i + 1)}. {que['display_type']}", value=str(i)
                )
        view.add_item(options)
        await interaction.response.edit_message(view=view)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.action_type == ActionInteraction.ACCEPT:
            ...
            embed = get_actions_embed(
                str(interaction.guild.id), self.app_name, ActionInteraction.ACCEPT
            )
        if self.action_type == ActionInteraction.DECLINE:
            editor = ActionDeclineEditorView(str(interaction.guild.id), self.app_name)
            embed = get_actions_embed(
                str(interaction.guild.id), self.app_name, ActionInteraction.DECLINE
            )
        await interaction.response.edit_message(embed=embed, view=editor)
...
class ApplicationStartButtonView(discord.ui.View):
    ...
    async def start_app(
        ...
        if questions == "error on get questions: application not found":
            await interaction.response.send_message(
                content="Application no longer exists", ephemeral=True
            )
            return
        if max_questions == 0:
            await interaction.response.send_message(
                content="Application doesn't have any questions", ephemeral=True
            )
            return
        ...
        embedd = discord.Embed(
            title=f"{interaction.guild.name} application: {app_name}",
            description="Hey! Your application has started. You have 300 seconds to complete it.",
        )
        embedd.add_field(
            value="Please note that answers longer than 1000 characters will be shortened",
            name="",
            inline=False,
        )
        embedd.add_field(
            value=f'You can cancel the application by answering "cancel" to any of the questions',
            name="",
            inline=False,
        )
        ...
        try:
            ...
            await interaction.response.send_message(
                content="Can't start application. Please allow direct messages from server members in your privacy settings.",
                ephemeral=True,
            )
            return

        await interaction.response.send_message(
            content=f"Application started {user.jump_url}", ephemeral=True
        )
        ...
        for i in range(0, int(max_questions)):
            try:
                ...
                response = await bot.wait_for(
                    "message",
                    check=lambda m: m.author == interaction.user and m.channel == user,
                    timeout=300,
                )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.action == "acc":
            ...
            try:
                ...
                await interaction.response.send_message(
                    content="Cannot send messages to this user.\nHe is likely no longer on this server.",
                    ephemeral=True,
                )
                return
            await user.send(f"Reason: {reason}")
            await interaction.response.send_message(
                content="Application accepted", ephemeral=True
            )
                ...
                if i["action_type"] == "add_role":
                    ...
                    try:
                        ...
                        await interaction.followup.send(
                            content=f"I was unable to add role `{role.name}`, I'm missing the permissions"
                        )
                else:
        ...
        if self.action == "dec":
            ...
            await user.send(f"Reason: {reason}")
            await interaction.response.send_message(
                content="Application declined", ephemeral=True
            )
